...
| **조회, 삽입, 삭제 (worst case)** | **O(N)** (불균형 트리일 경우) | **O(log N)**             |

B-tree는 **이진 탐색 트리**(BST)를 일반화한 트리이다.
- 이진 탐색 트리는 모든 노드의 왼쪽 서브 트리는 해당 노드의 값보다 작은 값들만 가지고, 모든 노드의 오른쪽 서브 트리는 해당 노드의 값보다 큰 값들만 가진다. 자녀 노드는 최대 두 개까지 가능하다.
...
> - **왜 Self-Balancing BST를 사용하지 않을까?**
> 
> B-tree를 인덱스에 쓰는 이유가 균형 트리(balanced tree)이기 때문이라면 AVL tree, Red-Black tree와 같은 Self-Balancing BST를 사용해도 되지 아닐까?
> 
...
| 구분           | **B-tree**                                                                                      | **Hash**                                                                        |
|----------------|--------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| **사용 범위**  | 대부분의 데이터베이스에서 일반적으로 사용                                                              | 주로 메모리 기반 데이터베이스에서 사용                                                |
| **인덱싱 방식** | 컬럼 값을 변형하지 않고 원래 값을 기반으로 인덱싱                                                      | 컬럼 값을 해시 값으로 변환하여 인덱싱                                                 |
| **특징**    | 범위 조회 및 정렬에 적합                                                                               | 정확한 일치 검색에 유리, 범위 검색 불가                                               |
...
- 
해시 테이블은 데이터 삽입, 삭제, 조회하는 시간복잡도가 O(1)이다. 왜
그렇다면 빠르게 조회하기 위해서는 해시 인덱스를 써야하는 것 아닌가 하는 의문이 들 수 있다.
