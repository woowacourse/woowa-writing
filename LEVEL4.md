...
인덱스는 조회 시 성능 개선에 유용합니다. 이론적으로는 충분히 이해합니다. 하지만 아직까지 실제로 눈에 띄는 효과를 직접 확인하지 못했습니다. 따라서 직접 1억건의 데이터를 쌓고 실험을 해보고자 합니다. 그 여정을 공유합니다.
...
컬럼에 인덱스를 걸지 않은 경우, 데이터베이스는 행 전체를 탐색합니다. 이를 `full scan`이라고 칭하며, 만약 테이블에 100억개의 데이터가 존재한다면 100억개의 데이터 전부를 탐색해 조건에 맞는 행인지를 판단합니다. 그만큼 많은 시간이 소요되겠죠.
...
...
```
위 질문을 반복해 답을 얻어낼 수 있습니다. 하지만 과연 이 방식이 효과적일까요? 77이라는 숫자를 찾기 위해 우리는 77번의 질문을 던져야 합니다. 무척 힘이 듭니다.
...
## 데이터베이스 세팅
그렇다면 모든 상황에서 인덱스의 효과가 보장될까요? 모든 상황에서 인덱스를 통해 성능을 개선할 수 있을까요? 1억건의 데이터를 삽입하고 인덱스를 걸어보며 눈으로 직접 확인해봅시다.

### 1. 테이블 스키마 정의
Member 테이블의 스키마를 아래와 같이 정의하고 로컬 환경에 테이블을 생성해주었습니다.
```sql
create table member (
...
한 컬럼에 인덱스를 걸었을 때, 13밀리초에서 1분으로, **약 4000배의 성능 저하**를 보입니다.
...
4. 조회의 성능 개선보다 쓰기의 성능 저하 정도가 더 크다. 따라서 조회 쿼리 실행량이 4배 이상 많을 때 인덱스를 걸어야겠다.
...
프로젝트에 기존 존재하던 쿼리에 인덱스를 적용했지만, 성공적인 결과를 거두지 못했습니다. 따라서 인덱스가 잘 걸리도록 쿼리를 작성해야겠다 다짐했습니다.
...
이번 경험을 통해, 쿼리를 작성하는 시점에서부터 인덱스를 고려하여 쿼리 작성 계획을 세우겠다 다짐했습니다. 저희 총대마켓에서 사용하는 쿼리문들에 대해서는 쿼리 최적화를 진행한 상태인데요. 해당 과정을 정리 후 여러분께 공유 드리겠습니다.
