...
현대의 웹 애플리케이션에서는 사용자 인증이 필수적입니다. 특히, 소셜 로그인 기능은 사용자에게 간편한 인증 수단을 제공하여 이탈률을 낮추고 사용자 경험을 향상시키는 중요한 요소로 자리잡고 있습니다. 

이러한 배경 속에서, 본 문서는 GitHub의 OAuth2.0을 활용하여 소셜 로그인 기능을 구현하는 과정과 그 과정에서 겪었던 다양한 트러블슈팅 경험을 공유합니다.

OAuth2.0은 인증 및 권한 부여를 위한 표준 프로토콜로, 다양한 서비스 간의 안전한 상호작용을 가능하게 합니다. GitHub OAuth2.0을 통해 개발자는 사용자 인증을 쉽게 구현할 수 있으며, 사용자는 자신의 GitHub 계정을 사용하여 간편하게 서비스를 이용할 수 있습니다. 

하지만 OAuth2.0을 처음 접하는 개발자들에게는 개념적인 이해와 함께 실제 구현 시에 발생할 수 있는 다양한 이슈들이 고민거리가 될 수 있습니다.

본 문서의 주요 대상 독자는 GitHub OAuth2.0을 활용하여 소셜 로그인 기능을 구현하고자 하는 서버 및 클라이언트 개발자들입니다. 이를 위해 독자는 OAuth2.0의 기본 개념과 작동 방식을 미리 이해하고 있어야 합니다. 

문서는 OAuth의 개념부터 시작하여, OAuth2.0의 구조와 동작 원리, GitHub OAuth2.0 API의 사용법을 상세히 설명합니다. 

각 섹션에서는 필요한 이론적 배경뿐만 아니라 실제 코딩 예제와 함께 개발하면서 겪었던 시행착오를 소개하여 독자들이 보다 실질적으로 이해할 수 있도록 돕고자 합니다.

특히, 문서에서는 GitHub의 OAuth2.0 API를 통해 소셜 로그인 기능을 구현하는 과정에서 발생한 여러 가지 트러블슈팅 사례를 다룹니다. CORS 문제, JWT 토큰 관리, refreshToken의 부재 등 다양한 이슈를 어떻게 해결했는지를 공유함으로써, 독자들이 비슷한 문제에 직면했을 때 유용한 참고 자료가 될 것입니다. 이러한 내용을 통해 독자들은 실전에서 활용할 수 있는 팁과 노하우를 얻을 수 있을 것으로 기대합니다.

마지막으로, 본 문서는 단순히 기술적인 설명을 넘어서, 실제 경험에 기반한 인사이트를 제공하는 것을 목표로 하고 있습니다. GitHub OAuth2.0을 통해 소셜 로그인 기능을 구현하고자 하는 모든 개발자들에게 이 문서가 많은 도움이 되기를 바랍니다. 소셜 로그인의 도입은 더 이상 선택이 아닌 필수인 시대에, GitHub OAuth2.0을 활용하여 더 안전하고 간편한 사용자 인증을 구현해보시기 바랍니다!
...
예를 들어, 클라이언트에서 API 서버로 요청을 보낼 때 CORS 정책으로 인해 요청이 차단되는 경우가 많았습니다. 이 문제를 해결하기 위해 서버에서 CORS 정책을 적절히 설정하고, Interceptor를 통해 Preflight 요청을 처리하는 방법을 적용했습니다. 
...
CORS 에러 처리하는 법
ㅠㅠ preflight는 또 뭔가요
와 같은 키워드들을 검색하며 각자 유익한 시간을 보내야 했습니다.

그리하여 알게 된 사실은 다음과 같습니다.

문제는 의외로 CORS 설정이 아니었습니다.
...
이때 해당 `handler`가 `Controller`인지 `HandlerMethod`인지 알 수 없는 상황에서 무조건 캐스팅을 하려고 하니 문제가 날 수밖에요.
...
그랬더니 잘 돌아가더라고요! 고작 세 줄만으로! *이럴 때마다 프로그래밍이 사랑스럽다 증오스럽다 하네요. ^_^*
...
- **에러 처리**: 인증 과정에서 발생할 수 있는 다양한 오류를 적절히 처리할 수 있는 방법을 마련해야 합니다. 예를 들어, 잘못된 토큰이나 만료된 세션에 대한 처리를 포함하여, 사용자에게 명확한 오류 메시지를 제공하는 것이 중요합니다.

- **사용자 경험**: 사용자에게 명확하고 간편한 인증 과정을 제공해야 합니다. 복잡한 절차나 불필요한 단계는 사용자 경험을 저하시킬 수 있으므로, 가능한 한 간단하게 구성하는 것이 좋습니다.

이처럼 OAuth를 활용하여 소셜 로그인을 구현할 때는 보안과 사용자 경험 모두를 고려한 신중한 접근이 필요합니다. 
이를 통해 안전하고 편리한 인증 기능을 제공할 수 있을 것입니다.
