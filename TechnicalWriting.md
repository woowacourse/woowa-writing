...
오픈소스 DBMS인 MySQL과 PostgreSQL은 각각 다른 특징을 지니고 있으며, 이 둘 중 선택은 프로젝트 요구 사항, 애플리케이션 특성, 그리고 개발팀의 경험과 선호도에 따라 달라집니다. 그런데도, 자신의 서비스에 MySQL과 PostgreSQL 중 어느 것이 더 적합할지 판단하기가 여전히 어렵습니다.
...
MySQL은 순수 관계형 데이터베이스로, 표준 데이터 유형만을 지원합니다. 이에 비해 PostgreSQL은 객체 관계형 데이터베이스로, 속성을 가진 객체로 저장할 수 있습니다. 이러한 객체는 Java 및 .NET과 같은 여러 프로그래밍 언어에서 일반적으로 사용되는 데이터 유형이며, 상위-하위 관계 및 상속과 같은 개념을 지원합니다.   
