...
우테코 크루의 인원은 총 150명 내외입니다. 우테코 7기 프리코스 참여자는 작년 기준 4천명 내외일 것입니다. 이 사용자들이 각자 10개씩 리뷰를 남겨서 4만건 이상의 리뷰가 DB에 쌓인다면, 이 인원이 동시에 조회한다면 사용자들은 어떤 경험을 하게될까요?

이제 조회 성능에 문제가 생길 것이라는 건 어렵지 않게 추측할 수 있습니다. 서비스를 바라보는 관점이 달라질 시점이 되었습니다. 오버 스펙을 준비하다는 걱정이 아닌, 자주 변화하는 서비스 규모, 이벤트에 유연하게 대처하기 위한 방법을 준비하는 단계에 다다른 것이죠.
...
상세 리뷰에서 질문지는 모든 사용자가 같은 질문지를 보게됩니다. 리뷰 작성의 템플이 바뀌어서 질문지가 변경되는 경우가 발생할 수 있으나 가능성이 낮습니다. 즉, 리뷰어의 서술형 답변처럼 어떤 사용자가 요청하냐에 따라 달라지는 데이터가 아닌  변경될 가능성이 적은 데이터라고 할 수 있겠네요.

이 때, DB 접근을 최대한 줄이는 것이 우리의 목표이기에 어렵지 않게 떠올릴 수 있는 방법이 있습니다.
바로, 한 번에 DB에서 어플리케이션으로 데이터를 가져오고 이후에는 그걸 계속 재사용하는, 캐싱입니다.

캐싱의 방식은 여러가지가 있습니다. 그 중 우리는 가장 간단하게 적용할 수 있는 스프링 캐시를 적용하기로 하였습니다.
```java
...
## 2단계: 리뷰 목록 조회 쿼리 최적화
1단계에서는 변경 가능성이 낮은 데이터에 대해 캐싱을 적용해보았습니다. 하지만,  변경 가능성이 낮은 질문지 데이터가 있는 반면, 사용자마다 서로 다른 데이터를 갖는 ‘리뷰 내용' 데이터도 있습니다.
