...
즉, 스프링은 데이터 계층의 예외 전환을 웹 환경의 응답 전환(Exception Translation) 으로 확장한 것이다.
...
이 흐름은 예외가 각 계층의 맥락에서 새롭게 해석되면서도 전체적으로 일관된 의미를 유지하는 구조를 보여준다. 데이터 계층, 서비스 계층, 웹 계층은 서로 다른 책임을 지니지만, 스프링의 예외 전환 체계 아래에서는 모든 실패가 “적절한 수준에서 의미 있게 처리되는 하나의 언어 흐름” 으로 이어진다.
