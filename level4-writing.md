...
이 글은 “스프링은 왜 이런 방식으로 예외를 다루는가?”라는 질문에서 출발한다. 자바의 Checked Exception이 가진 철학과 한계를 짚고, 스프링이 이를 예외 전환(Exception Translation) 이라는 개념으로 어떻게 확장했는지 살펴본다. 그리고 더 나아가, 예외를 정의하는 관점에 대해서도 함께 다룬다.
...
이 구조는 이전 언어의 오류 처리 방식을 개선하기 위해 설계되었다. C++에서도 파일 접근 실패나 네트워크 오류처럼 예상 가능한 실패를 다루기 위한 장치가 존재했다. 함수가 실패하면 -1이나 null 같은 특수한 리턴값으로 그 사실을 알렸고, 호출자가 이를 직접 확인해야 했다. 하지만 이 검사를 누락하면 실패는 조용히 무시되었고, 프로그램은 오류를 인식하지 못한 채 계속 실행되었다.
...
즉, 스프링의 예외 전환은 단순히 기술 의존성을 숨기기 위한 추상화가 아니다. 복구 불가능한 예외를 모든 코드에서 강제로 처리하지 않고, 의미가 필요한 계층에서만 해석할 수 있도록 구조화한 설계 방식이다. 자바의 Checked Exception이 “모든 실패를 처리하라”고 강제했다면, 스프링의 Exception Translation은 “실패를 적절한 계층에서 의미 있게 다뤄라”는 방향을 제시한다.
...
이처럼 스프링은 예외를 잡아 HTTP 응답으로 전환하는 전체 흐름을 프레임워크 수준에서 담당한다. 개발자는 “무엇이 실패했는가”만 정의하면 되고, “그 실패를 어떻게 응답으로 표현할 것인가”는 스프링이 대신 처리한다. 그러나 이 구조는 단순히 기술적인 편의 기능에 그치지 않는다. 스프링이 데이터 접근 계층에서 SQLException을 DataAccessException으로 전환했던 것처럼, 웹 계층에서는 내부 예외를 “의미 있는 응답”으로 전환함으로써 서버 내부의 실패를 외부 세계와 소통 가능한 언어로 변환하는 철학적 일관성을 보여준다.

즉, 예외 전환은 데이터 계층에만 머무는 개념이 아니라, 스프링 전반에 걸친 실패 해석 구조의 일부다.

## 6.2 계층 간 의미의 연속성
스프링의 예외 전환 체계는 단일 계층의 문제를 해결하기 위한 기능이 아니다. 데이터베이스, 서비스 로직, 웹 응답으로 이어지는 전체 애플리케이션의 실패 흐름을 하나의 일관된 의미로 연결하는 구조적 원리다.
...
데이터 계층은 기술별 예외를 추상화하고, 서비스 계층은 이를 비즈니스 규칙 위반으로 해석하며, 웹 계층은 다시 클라이언트가 이해할 수 있는 형태로 변환한다. 이 과정을 통해 같은 실패라도 계층마다 다른 의미로 재해석되지만, 전체적으로는 일관된 실패 의미를 유지한다.

예를 들어, 데이터베이스 제약 조건 위반은 다음과 같은 흐름으로 전환된다.
...
스프링은 대부분의 실패를 자동으로 처리하고 일관된 구조로 전환하지만, 예외를 대신 정의해주지는 않는다. 프레임워크는 실패를 전달할 수 있으나, 그 실패가 “예외로서 존재할 필요가 있는가”는 개발자의 판단에 달려 있다.

즉, 이제 질문은 “예외를 어떻게 다룰 것인가”가 아니라 “어떤 실패를 예외로 다뤄야 하는가”로 바뀐다.

## 7.1 복구 가능한 실패

복구 가능한 실패는 예외 전환이 필요한 대표적 경우다. 이 실패는 단순한 오류가 아니라, 사용자나 상위 계층이 행동을 수정하여 회복할 수 있는 신호이기 때문이다.

예를 들어, 사용자의 입력 오류, 존재하지 않는 리소스 접근, 중복 요청 등은 모두 “다시 시도하거나 수정할 수 있는 실패”다. 이런 경우에는 예외를 정의하고 명시적으로 전환해야 한다.
...
이 예외는 단순한 에러가 아니라, “복구 방법이 존재한다”는 사실을 코드와 응답 양쪽에서 드러낸다.
...
복구할 수 없는 실패는 상황이 다르다. 데이터베이스 연결 단절, 외부 API 타임아웃, 네트워크 장애 등은 시스템 내부에서 해결할 수 없는 실패이며, 복구보다는 인지와 격리가 중요하다.

이런 경우에는 예외를 정의하기보다, 상위 계층이 의미를 해석해야 하는 경우에만 전환이 필요하다.
...
이 코드는 복구를 시도하지 않지만, SQLException을 DataAccessException으로 전환함으로써 상위 계층이 “데이터 접근 실패”라는 의미를 일관되게 인식할 수 있다.

그러나 이런 예외를 클라이언트까지 그대로 전달할 필요는 거의 없다. 이 경우에는 내부 로그로만 남기고 일반적인 500 응답으로 대체하는 편이 적절하다.

즉, 복구 불가능한 실패는 전환의 필요성이 있을 때만 전환되어야 한다. 상위 계층이 의미를 해석할 필요가 없다면, 예외 정의 자체가 불필요하다. 의미가 소비되지 않는 예외는 단순한 노이즈다.

## 7.3 예외 정의를 위한 점검 기준

예외 정의는 단순히 클래스를 추가하는 일이 아니다. 시스템이 실패를 어떻게 해석하고 어디까지 전달할지를 결정하는 설계 행위다. 따라서 예외를 정의하기 전에는 반드시 다음 두 가지를 점검해야 한다.

1. 이 실패는 복구할 수 있는가? - 복구가 가능하다면 예외를 명시적으로 정의하고 전환해야 한다.
2. 복구할 수 없다면, 누가 이 실패를 이해해야 하는가? - 상위 계층이 의미를 해석해야 한다면 전환이 필요하다. 그러나 어느 계층에서도 소비되지 않는다면 로그로만 남겨야 한다.

결국 복구 가능한 실패는 드러내고, 복구 불가능한 실패는 의미가 필요한 곳에서만 전환한다. 예외는 모든 오류를 잡기 위한 장치가 아니라, 전달할 가치가 있는 실패를 의미로 바꾸는 도구다.