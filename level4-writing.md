...
이 과정은 단순한 기술적 매핑이 아니라, 데이터 계층에서 시작된 실패의 의미가 서비스와 웹 계층을 거치며 연속적으로 전환되는 과정이다
...
데이터 계층이 기술별 예외를 추상화하듯, 웹 계층은 예외를 표준화된 응답 구조로 추상화한다.
이렇게 전환된 실패는 일관된 형태로 전달되지만, 각 계층은 자신의 책임에 따라 그 의미를 해석하고 대응한다.

즉, 스프링은 모든 계층에서 동일하게 예외를 처리하도록 강제하지 않고, 실패를 적절한 계층에서 의미 있게 다루도록 유도한다.
...
이 구조는 예외가 계층을 거치며 다른 형태로 표현되더라도, 결국 같은 실패가 일관된 의미로 해석되도록 보장한다. 데이터 계층에서는 기술적 오류로, 서비스 계층에서는 비즈니스 규칙 위반으로, 웹 계층에서는 HTTP 응답으로 표현되지만, 스프링의 예외 전환 체계 아래에서는 이 모든 과정이 하나의 일관된 실패 흐름으로 이어진다.

이제 스프링의 예외 전환 체계를 이해했다면, 프레임워크 내부의 철학을 넘어 개발자가 직접 다뤄야 할 영역으로 시선을 옮겨야 한다.
