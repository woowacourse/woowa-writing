...
그렇다고 해서 모든 실패가 예외가 되는 것은 아니다. 예외는 예상 가능한 실패(predictable failure) 를 표현하기 위한 수단이다. 예를 들어, 파일이 존재하지 않거나 네트워크가 끊기는 상황은 언제든 발생할 수 있다. 이런 실패는 충분히 예상 가능한 범주에 속하며, 코드 차원에서 미리 대비해야 한다. 반면 프로그래밍 오류나 버그처럼 개발자가 통제해야 할 문제는 예외로 처리할 대상이 아니다. 그것은 코드의 수정으로 해결해야 할 문제다.

즉, 예외는 실패 전체를 다루는 메커니즘이 아니라, 그중에서 예상 가능한 실패를 코드 수준에서 표현하기 위한 구조다. 자바의 예외 체계는 이 개념을 중심으로 구성되어 있다.
