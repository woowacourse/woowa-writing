...
> “논점은 Checked Exception을 없앨 것인가가 아니라,  
>  그 예외가 실제로 복구 가능한가를 어떻게 표현할 것인가이다.”
...
마찬가지로, 데이터 접근 계층에서는 이 철학이 가장 먼저 뚜렷하게 드러났다. 데이터베이스 연동 과정은 다양한 기술 스택과 예외 체계를 다루는 대표적인 복잡 영역이었고, 스프링은 이 영역에서 “기술별 예외를 하나의 언어로 통합한다”는 목표를 실현하기 시작했다.
...
즉, 스프링의 예외 전환은 기술을 숨기기 위한 추상화가 아니라, 실패를 일관된 관점에서 해석하기 위한 추상화다. 자바의 Checked Exception이 “실패를 반드시 처리하라”고 강제했다면, 스프링의 Exception Translation은 “실패를 일관된 의미로 이해하라”고 제시했다.

# 6. 웹 환경으로의 확장

스프링의 예외 설계는 데이터 접근 계층에 한정되지 않는다. “실패를 일관된 언어로 표현한다”는 철학은 컨트롤러와 HTTP 응답으로 이어지는 웹 환경으로 확장되었다. 데이터베이스의 SQLException이 DataAccessException으로 전환되듯, 웹의 예외 또한 클라이언트가 이해할 수 있는 형태로 전환된다.

이때 스프링이 수행하는 일은 단순히 에러를 응답으로 내보내는 것이 아니다. 프레임워크 수준에서 예외의 의미를 계층별로 해석하고, 서로 다른 기술 간의 실패를 하나의 언어로 전환하는 일관된 구조를 유지하는 것이다. 이 철학적 확장이 바로 “웹 환경의 예외 전환(Exception Translation)”이다.

## 6.1 웹 응답으로의 전환

스프링 MVC에서 컨트롤러는 메서드의 결과를 HTTP 응답으로 전환한다. 예외가 발생하면 정상적인 반환 흐름이 끊기므로, 프레임워크는 예외를 HTTP 응답 형태로 전환(translate) 해야 한다.

이 구조는 단순히 기술적 매핑이 아니라, 데이터 계층에서 시작된 실패의 의미가 서비스와 웹 계층을 거치며 연속적으로 번역되는 과정이다.
...
데이터 계층에서 기술별 예외를 추상화하듯, 웹 계층에서는 예외를 표준화된 응답 구조로 추상화한다. 이 덕분에 사용자는 실패의 원인을 몰라도 의미를 이해할 수 있고, 개발자는 계층 간 의존 없이 일관된 방식으로 예외를 다룰 수 있다.
...
그리고 이 예외는 컨트롤러 단에서 다음과 같이 HTTP 응답으로 변환된다.
...
이 예시는 예외가 각 계층의 맥락에 맞게 의미를 새롭게 해석하면서도, 전체적으로 일관된 언어로 유지되는 과정을 보여준다. 데이터 계층, 서비스 계층, 웹 계층은 서로 다른 책임을 지니지만, 스프링의 예외 전환 체계 아래에서는 모든 실패가 하나의 의미 흐름으로 이어진다.

# 7. 예외 정의 기준

스프링은 대부분의 실패를 자동으로 처리하고 일관된 구조로 전환하지만, 모든 예외를 대신 정의해주지는 않는다. 프레임워크는 실패를 전달할 수 있으나, 그 실패가 “예외로서 존재할 필요가 있는가”는 개발자의 판단에 달려 있다.

즉, 이제 질문은 단순히 “예외를 어떻게 다룰 것인가”가 아니라 “어떤 실패를 예외로 전환할 가치가 있는가”로 바뀐다.
...
이 코드는 복구를 시도하지 않지만, SQLException을 DataAccessException으로 전환함으로써 상위 계층이 데이터 접근 실패라는 의미를 일관되게 인식할 수 있도록 한다.

그러나 이 예외를 클라이언트까지 그대로 전달하는 것은 불필요하다. 이 경우에는 내부 로그로만 남기고 일반적인 500 응답으로 대체하는 편이 적절하다.
...
즉, 복구 불가능한 실패는 전환을 최소화하고, 의미를 보존하는 선에서만 표현해야 한다.
