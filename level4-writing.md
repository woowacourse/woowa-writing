...
1. **예상 가능한 실패를 명시적으로 드러내라.**
    
    예외를 메서드 시그니처에 선언함으로써 호출자가 대비할 수 있게 한다.
    
2. **복구 가능한 실패는 직접 처리하라.**
    
    강제된 `try-catch`를 통해 시스템 안정성을 높인다.
    
3. **비정상 상황을 정상 제어 흐름 안으로 편입하라.**
    
    오류를 숨기지 않고 프로그램 구조 안에서 관리한다.
