...
| 작업 흐름 | 1. 행동 -> 2. 로더 표시 -> 3. 서버 요청 -> 4. 응답 -> 5. UI 업데이트 | 1. 행동 -> 2. UI 업데이트 -> 3. 서버 요청 -> 4. 상태 조정 (필요시) |
| 구현 복잡도 | 낮음 | 높은 (롤백을 위한 상태 관리 필요) |
| 이상적 사용 사례 | 금융 거래, 중요한 양식 제출 | ‘좋아요’, 댓글, 장바구니 추가, 채팅 메시지 |

두 방식 중 어느 것이 절대적으로 우월하다고는 할 수 없으며, 적용할 기능의 맥락에 따라 신중하게 선택해야 한다. 예를 들어, 데이터 정확성이 무엇보다 중요한 금융 거래에는 전통적 방식이, 바른 반응이 중요한 소셜 미디어 기능에는 낙관적 UI가 적합하다.
...
이번 프로젝트를 통해 “기능이 동작하는 것”과 사용자에게 좋은 경험을 주는 것” 사이의 간극을 깊이 이해하게 되었다. 사용자의 입장에서 ‘느라다’고 느끼는 순간을 포착하고, 이를 기술적인 해법으로 풀어내는 과정은 매우 값진 경험이었다.
...
이 글이 많은 사람들의 앱을 한 단계 더 나은 사용자 경험으로 이끄는 데 작은 도움이 되었으면 좋겠다.