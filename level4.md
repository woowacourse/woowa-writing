...
이를 구현하는 대표적인 방법으로는  
**폴링(Polling)**, **롱 폴링(Long Polling)**, **웹소켓(WebSocket)** 이 잘 알려져 있습니다. <br>
하지만 이들 방식이 항상 최적의 선택은 아닙니다. <br> 
서버에서 클라이언트로 **단방향 실시간 데이터 스트림**을 전송해야 하는 상황이라면, <br>
**SSE(Server-Sent Events)** 가 더 단순하고 효율적인 대안이 될 수 있습니다.
...
웹소켓처럼 별도의 프로토콜이나 핸드셰이크 과정이 필요하지 않고, 표준 HTTP로 동작하기 때문에 브라우저 호환성, 프록시 통과성, 구현 단순성 면에서 큰 장점을 가집니다. <br>
...
실제로 버퍼를 읽고 이벤트 데이터를 처리하는 로직은 `ServerSentEventReader` 내부에 구현되어 있는데, 로직이 복잡하게 구성되어 있어 깊게 이해하지 않아도 됩니다. <br>
...
`EventSourceListener`의 콜백 메소드로 연결이 되었을 때, 실패했을 때, 연결이 끊겼을 때를 처리하고 있으며 `ServerSentEventReader`를 통해 이벤트를 수신 및 처리하고 있습니다. <br>
...
```
`EventSource`를 생성하는 데 필요한 `OkHttpClient`를 생성자로 주입받고, `EventSourceListener`를 상속받아서 이벤트 처리 로직을 내부에서 구현했습니다. <br>
...
또한 이벤트 DTO는 `EventStateResponse` 타입을 상속받게 하여 하나의 Flow에 담고, Presentation 계층에서 이벤트 유형에 따른 처리를 쉽게 할 수 있도록 하였습니다. <br>
...
이는 Domain Layer가 Data Layer에 의존하는 것을 방지하기 위함이며, 구글 권장 아키텍쳐를 따른 설계입니다. <br>
