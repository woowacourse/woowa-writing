...
## 1. 주제 선택 이유
프로그래밍은 항상 선택의 연속이다.  
선택할 항목에 따라 장점과 단점을 비교해야 하고, 언제나 그에 따른 `Trade off`가 발생한다.  
인프라 역시 마찬가지이다.  
토독토독 서비스 팀 프로젝트를 시작하며, 인프라 영역에서 어느 것을 선택하는 것이 좋을 지 가장 궁금했던 부분이 EC2와 RDS 선택 지점이었다.  
이 글은 AWS 인프라를 처음 접하거나, 접한 지 얼마 되지 않은 사람들을 위한 글이다.  
EC2에 직접 DB를 설치해야 할 지, RDS를 사용해야 할 지 그 선택에 필요한 정보를 제공하며 토독토독 서비스에서는 어떤 선택을 했는지 적어보려 한다.  

## 2. AWS EC2
### a. AWS EC2란?
<img src="./image/ec2.png" width="300" height="200">

AWS EC2는 **A**mazon **W**eb **S**ervices의 **E**lastic **C**ompute **C**loud의 약자로, 쉽게 말 해 `AWS에서 제공하는 가상 컴퓨터(서버)`이다.  
과거 On-Premise 서버 환경처럼 물리적인 서버 컴퓨터를 직접 구매하는 것이 아니라, AWS에서 클릭 몇 번으로 서버를 생성할 수 있다.  
AWS에게 서버 컴퓨터를 빌리는 개념으로도 생각할 수 있기 때문에, 사용한 만큼만 비용을 지불하면 된다.  

초기 비용과 전기, 네트워크, 인력 등 유지보수 비용이 많이 들었던 과거와 비교하여, 스타트업이나 개인 개발자도 쉽게 서버 컴퓨터를 대여하여 사용할 수 있게 되었다.  
다양한 인스턴스의 종류 중 필요에 따라 선택하여 사용할 수 있으며, 원하는 운영 체제(Linux, Windows) 뿐만 아니라 애플리케이션, 서비스, 부팅 파티션 크기까지 선택할 수 있다.  
오늘 날 EC2는 웹 서버나 API 서버, 실험용 서버 등 대부분 서비스의 시작점이다.

### b. AWS EC2 인스턴스의 종류
AWS에서는 사용자의 필요에 따라 사양을 선택할 수 있도록 다양한 인스턴스 유형을 제공하고 있다.  
인스턴스 유형은 CPU, 메모리, 스토리지, 네트워킹 용량 등의 다양한 조합으로 구성된다.  
AWS의 공식 페이지에 따르면 크게 범용, 컴퓨팅 최적화, 메모리 최적화, 가속화된 컴퓨팅, 스토리지 최적화, HPC(고성능 컴퓨팅) 최적화로 나뉘며, 아래 표와 같이 정리해볼 수 있다.  
...
토독토독 서비스와 같이 이제 막 시작한 웹/앱 어플리케이션 서비스의 WAS 서버는 범용의 t4g 타입을 선택하는 것이 일반적이다.  

## 3. AWS RDS
### a. AWS RDS란?
<img src="./image/rds.png" width="300" height="200">

AWS RDS는 **A**mazon **W**eb **S**ervices의 **R**elational **D**atabase **S**ervice의 약자로, `데이터베이스를 완전 관리형으로 사용할 수 있는 서비스`이다.  
완전 관리형이 무엇일까?  
RDS를 사용하지 않는다면 사용자는 EC2에 직접 원하는 데이터베이스를 설치하고, 여러 설정들을 관리하며 사용해야 한다.  
그에 비해 RDS를 사용하게 되면 PostgreSQL, MySQL, MariaDB, Oracle, SQL Server(MSSQL), Amazon Aurora 등 다양한 엔진 중 하나를 선택하여 관리 부담 없이 데이터베이스를 운영하고 사용할 수 있다.  

### b. AWS RDS의 장점
관리 부담이 없다는 것은 책임이 분리되어 있다는 의미로 이해할 수 있겠다.  
사용자는 데이터베이스의 관리와 관련된 측면은 신경쓰지 않고 데이터베이스를 사용할 수 있다. 이 것이 RDS의 가장 큰 장점이라고 볼 수 있다.  

또, 컴퓨팅 규모나 스토리지 규모를 빠르게 조정할 수 있으며 DB 인스턴스의 복제본을 생성하여 대량 읽기 트래픽을 처리할 수 있다는 장점이 있다.  
게다가 설정만 한다면 자동으로 스냅샷 백업과 복원이 가능하고, Multi-AZ 구성으로 장애가 발생했을 때 자동 Failover가 가능해 고가용성을 지원한다.  
관리형 서비스인 만큼 별도의 모니터링 도구를 붙이지 않아도 AWS의 CloudWatch를 통해 성능 모니터링도 가능하다.  

### c. AWS RDS 데이터베이스의 종류
AWS RDS 데이터베이스는 위에 언급했듯이 다양한 엔진 중 하나를 선택할 수 있다.  
엔진의 종류는 아래 표와 같이 정리할 수 있다.
...
위 그림은 On-premise 서버, AWS EC2, AWS RDS를 사용했을 때 사용자와 AWS의 책임 분담을 나타낸다.  
그림에서도 확인할 수 있듯, EC2에서는 RDS보다 사용자의 책임 분담이 더 많고, RDS는 AWS의 책임 분담이 더 많다.  
그만큼 사용자는 데이터베이스 관리보다 어플리케이션 개발에 더 힘 쏟을 수 있다는 뜻이다.  

EC2를 사용할 때와 RDS를 사용할 때, 각각의 차이점은 무엇인지 아래 표와 같이 정리할 수 있다.
