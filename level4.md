...
안드로이드 앱 개발에서는 사용자 몰래 백그라운드에서 실행돼야 하는 작업들이 많다. 예를 들어 정기적으로 서버와 데이터를 동기화하거나, 앱이 완전히 종료되어도 완료되어야 하는 작업들이 있다. 이 글에서는 이러한 **지연 가능한 백그라운드 작업**을 안정적이고 효율적으로 관리하는 Jetpack 라이브러리 **WorkManager**를 다룬다. WorkManager가 등장한 배경과 기본 개념, 실제 서비스 적용 사례, 코드 예시, 권장 패턴과 한계를 정리하며 백그라운드 작업을 공부해 보자.
...
WorkManager는 다양한 안드로이드 기기와 OS 버전에서 **지연이 가능한(deferrable)** 백그라운드 작업을 **실행**시키는 것을 보장하는 **표준 도구**다. 일회성/주기성 작업, 조건부 실행, 체이닝, 재시도, 고유 작업 이름 등 풍부한 기능을 적절히 조합하면, 앱이 종료되거나 기기가 재시작되어도 안정적으로 백그라운드 로직을 운영할 수 있다. 실제로 WorkManager는 안드로이드 팀이 권장하는 백그라운드 작업 해법으로, 이전의 여러 API들을 통합한 강력한 라이브러리다.
