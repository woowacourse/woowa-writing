...
WorkManager는 다양한 안드로이드 기기와 OS 버전에서 **지연이 가능한(deferrable)** 백그라운드 작업을 **실행**시키는 것을 보장하는 **표준 도구**다. WorkManager는 일회성·주기성 작업, 조건부 실행, 체이닝, 재시도, 고유 이름 관리를 지원해, 앱 종료나 재부팅 이후에도 안정적으로 작업을 이어간다. 실제로 WorkManager는 안드로이드 팀이 권장하는 백그라운드 작업 해법으로, 이전의 여러 API들을 통합한 강력한 라이브러리다.
