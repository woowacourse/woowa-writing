...
Lovable에서 동작하는 프로토타입은 새로운 기능 추가, 유지 보수, 보안, 모니터링 등 제약이 많기 때문에 결국 자체 인프라 환경으로 마이그레이션해야 합니다.

![](https://velog.velcdn.com/images/heiler/post/c7f898f5-e8b2-40cb-ac05-108e0d96203e/image.png)

`기획 → AI를 활용한 프로토타이핑 → 자체 인프라로 마이그레이션 및 런칭` 사이클 과정에서 새로운 프로토타입을 만들 때마다 이런 의문이 들었습니다.

> 인프라를 매번 수동으로 만들어야 할까?

운영/모니터링이 가능한 최소 비용의 인프라 설계는 어떤 프로토타입이든 비슷할 것이라 생각했고, 이미 설계해 놓은 인프라를 템플릿으로 만들어 쉽게 재현할 수 있는 방법을 찾던 중에 Terraform을 접하게 됐습니다.

최근에 "코스잇다"라는 프로젝트를 새로 시작하게 됐는데요. 이 프로젝트에 Terraform을 도입하면서 "이 정도만 이해하고 있어도 누구든 충분히 Terraform을 사용할 수 있겠다" 느꼈던 내용들을 정리해 보았습니다.

Terraform을 처음 접하거나, 제 상황처럼 작은 규모의 프로젝트에 도입하려는 분들께 도움이 되길 바랍니다. 😄
