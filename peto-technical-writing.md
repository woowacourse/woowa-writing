...
공식 문서에 따르면 **Android 플랫폼은 “사용 가능한 메모리가 있다는 것은 낭비다”** 라는 전제하에 작동합니다.  
따라서 Android 기기는 항상 사용 가능한 모든 메모리를 활용하며, **거의 메모리가 없는 상태로 실행**되는 경우가 많습니다.

> Android Runtime(ART)과 Dalvik VM은 **페이징(Paging)** 및 **메모리 매핑(Memory Mapping)** 기법을 사용해 메모리를 관리합니다.

이를 이해하기 위해서는 먼저 **프로세스의 메모리 할당 방식**을 알아야 합니다.
...
프로세스의 **할당과 해제가 반복**되면, 메모리 내에 사용 중인 영역과 빈 공간(hole)이 불규칙하게 섞입니다.  
이로 인해 **충분한 총 여유 공간이 있어도 연속된 공간이 없어** 프로세스를 적재하지 못하는 현상이 발생합니다.
...
<img width="600" height="300" alt="image" src="https://github.com/user-attachments/assets/dfcc2755-9b38-4a46-9791-c015264f45e1" />

할당 단위가 고정된 경우 프로세스 크기가 단위의 배수가 아닐 때 **마지막 블록의 일부가 낭비되는 현상**입니다.
