...
Spring Data JPA과 MySQL을 사용하는 프로젝트에서 쿼리 성능 측정 과정을 정리한 글입니다. 따라서 MySQL의 DML과 인덱스, Spring Data JPA 기본적인 이해가 필요합니다.

# 미션 등장

‘**핵심 테이블에 대량의 데이터 생성 및 성능 테스트’** 미션을 받았습니다.

미션 내용을 읽고 다음 작업을 계획을 세웠습니다.
...
더미 데이터 삽입은 프로시저 SQL를 사용하거나, 프로그래밍 언어를 통해 진행합니다.

성능 측정은 MySQL의 EXPLAIN, EXPLAIN ANALYZE 명령어를 사용하거나, 웹 어플리케이션 서버 요청 후 응답 시간을 확인하는 방법이 있습니다.

웹 어플리케이션 서버 요청 후 응답 확인은 네트워크 시간과 서버 로직 시간이 포함됩니다. 

따라서 순수 데이터베이스 동작 속도를 확인할 수 없습니다.

순수 데이터베이스 동작 시간을 확인하려면 데이터베이스 내부에서 동작 시간을 확인해야 합니다. 

`EXPLAIN`, `EXPLAIN ANALYZE` 는 각각 데이터베이스 쿼리(Query) 분석과 쿼리 성능 분석에 사용합니다.
