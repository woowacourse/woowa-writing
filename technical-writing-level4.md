# 좋은 답은 좋은 질문에서 시작된다 — Claude 문서로 본 프롬프트 설계법

> 같은 AI를 쓰는데, 왜 결과는 이렇게 다를까?
답은 ‘어떻게 질문하느냐’에 있다.
>
...
코드를 점검하고 테스트를 작성할 때뿐 아니라, 정보 탐색과 글 정리에도 AI를 사용했다.
...
프론트엔드를 맡을 사람이 없었기에, 우리는 *Lovable*과 *Cursor*를 활용해 프론트를 구현하기로 했다.

문제는 제한된 시간과 자원이었다. *Lovable*에는 코인 5개 제한이 있어, 한 번의 시도로 원하는 결과를 얻어야 했다.
...
프롬프트 엔지니어링(Prompt Engineering)은 AI가 올바르게 사고하고 응답하도록 입력을 설계하는 기술이다.
...
바로 Anthropic(앤트로픽)에서 작성한 [Claude Prompt Engineering Guide](https://docs.claude.com/ko/docs/build-with-claude/prompt-engineering/overview)였다.

---

## **5️⃣ Claude Docs: 프롬프트 설계의 핵심**

Anthropic의 *Claude Prompt Engineering Guide*는 AI의 사고를 설계하는 법을 가장 체계적으로 제시한 문서 중 하나다.

이 문서는 “AI를 잘 쓰는 요령”이 아니라, AI에게 논리적으로 사고하게 만드는 구조적 대화법을 다룬다.

즉, AI의 성능은 모델 자체보다 입력의 설계(Input Design) 에 달려 있다는 점을 강조한다.

해당 문서에는 다양한 프롬프트 설계 원칙이 담겨 있지만, 그중에서 ChatGPT, Cursor 등 다른 모델에도 적용할 수 있다고 판단한 다섯가지 원칙을 소개해 보겠다.
...
이 방식을 사고 연쇄(CoT, Chain of Thought)라고 부른다.
...
단순히 “도와줘”가 아니라, “너는 Lovable 전용 프롬프트 엔지니어야. 사용자의 입력을 Lovable이 이해할 수 있는 구조로 변환해.” 처럼 역할을 제한했다.
...
또한 “동일한 Build Doc → 동일한 결과”를 보장하기 위해 구체적인 스타일·이름·구조를 명시하도록 했다.

### ⑥ 디버깅 가능성

AI가 실패했을 때, “다시 처음부터 요청”하지 않아도 되도록 프롬프트 내에 AI가 스스로 오류를 진단하고 수정하도록 유도하는 구조(Self-debugging Loop)를 작성할 수 있게 설계했다.

### **▶️ 리버스 프롬프팅을 통해 프롬프트 라이브러리 만들기**

### **🔄 리버스 프롬프팅이란?**

`input → process → output` 과정을 통해서 결과가 도출되는데,
프롬프트를 작성할 때 output, 즉 결과물을 가지고 그 결과물을 만들어낸 프롬프트를 거꾸로 추적하며 재구성하는 것이다.

이때, output은 만들어낸 결과물 중 가장 마음에 드는 것일 수도 있고, 오픈 소스일 수도 있다.

이전에는 좋은 결과를 얻은 날이면, 그저 “오늘 운이 좋았다”라고 넘겼다.

하지만, 그 ‘운’을 재현하기 위해 우리는 ‘운을 복제할 수 있는 실험’을 시작했다.

프롬프트 생성기로 만든 프롬프트를 Lovable에게 주어 얻어낸 결과 중 가장 마음에 드는 부분을 리버스 프롬프트했다.

이 과정을 반복하면서, 우리 팀만의 **프롬프트 라이브러리**가 만들어졌고, 각 상황에 맞는 최적의 템플릿을 재사용할 수 있게 되었다.

### **▶️ 팀의 프롬프트 학습 패턴: HERA**

이 모든 과정을 정리하면서 우리는 AI와 협업하는 흐름에 이름을 붙였다.

> HERA는 Hypothesis–Experiment–Reverse–Apply의 약어로, 프롬프트 실험을 체계화한 팀 내부 프로세스 입니다.
>
...
AI가 발전하면서, 이제 우리는 많은 일을 AI를 통해 해낼 수 있게 되었다.

개발자에게 AI는 선택이 아닌 필수적인 역량이 되었다고 생각한다.

AI를 잘 활용하는 법을 익히면, 단순히 생산성을 높이는 수준을 넘어 자신의 역량을 극대화할 수 있다.

같은 도구를 쓰더라도 어떻게 사용하느냐에 따라 AI는 단순한 보조 도구가 되기도 하고, 때로는 뛰어난 동료가 되기도 한다.

그렇기에 우리는 AI를 ‘사용하는 법’만이 아니라, AI와 ‘함께 일하는 법’을 배워야 한다.

좋은 협업이 사람 사이의 이해와 존중에서 시작되듯, AI와의 협업 역시 ‘언어를 통한 이해’에서 출발한다.

우리가 사람과의 협업을 다듬어 왔듯, 이제는 그 노력을 AI와의 협업에도 기울여야 한다.

AI는 자동화 도구를 넘어, 언어를 통해 함께 사고할 수 있는 협업 파트너다.

결국, 좋은 결과는 언제나 좋은 질문에서 시작한다.

우리가 언어로 사고를 설계할 수 있을때, AI와의 협업은 비로소 한단계 깊어질 것이다.
