...
> **리버스 프롬프팅이란?**
> 
> AI는 `input → process → output` 과정으로 결과가 도출되는데,
> 
> 우연히 운을 통해서 좋은 output을 얻는 경우가 있다.
> 
> 이러한 output을 가지고 그 output을 만들어낸 input을 추적하여 재구성해 해당 output를 재현해낼 수 있는 input을 얻는 것이 리버스 프롬프팅이다.
> 
> 이때, output은 우연히 얻게된 좋은 output 일 수도, 오픈 소스일 수도 있다.
