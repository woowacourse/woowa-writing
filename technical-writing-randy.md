...
그 결과, 여로 클라이언트에서의 요청이 하나의 IP에 할당된 요청 제한 기준을 공유하게 되었고,
...
이 관찰을 통해 “같은 Wi-Fi에 연결된 호스트들은 모두 동일한 IP 주소를 사용해 요청을 보내는 것일까?”라는 의문이 생겼습니다.
...
본격적인 원인 분석에 앞서, 이해를 돕기 위해 학습한 배경 지식을 정리하고자 합니다.
...
할당받아 인터넷에 사용하고 있습니다. 실제로 ISP와 인터넷 서비스를 계약하면 제공되는 공유기에 공인 IP 
주소가 할당되며, 이러한 공유기를 통해 저희는 핸드폰이나 노트북으로 인터넷에 접속할 수 있게 됩니다.

실제로 ISP로부터 할당받은 공인 IP 주소는 어떻게 확인할 수 있을까요?
일반적인 가정용 공유기에 할당된 공인 IP 주소는 ISP에서 제공하는 공유기 관리 페이지를 통해 확인할 수 있습니다.
...
즉, NAT은 사설 네트워크와 외부 네트워크 간의 주소 변환 중개자 역할을 수행하며,
...
- 내부 호스트에서 외부로 요청을 보내는 경우 (사설 IP 주소 \-\> 공인 IP 주소)
    ...
    - 선택된 사설 IP와 공인 IP 쌍을 NAT 변환 테이블에 저장합니다.
...
즉, NAT이 사설 IP 주소와 공인 IP 주소를 1:1로 매핑하는 방식이라면, NAPT는 포트 번호를 함께 사용하여 
사설 IP 주소와 공인 IP 주소의 N:1 매핑을 가능하게 하는 기술이라고 할 수 있습니다.
...
더 나아가 Docker 컨테이너에 할당된 IP 주소가 모두 사설 IP 범위에 속한다는 점을 깨닫았으며,
...
그리고 진행 중이던 악의적인 트래픽 제한 작업은 팀 차원에서 잠시 중단하게 되었습니다.
...
요청 빈도가 높다는 특성상 단순한 IP 기반 제약은 더욱 부적절하다고 판단했습니다.
