...
결국 서버 내부에서는 회원가입이 성공했음에도 불구하고, 요청이 실패한 것처럼 보이는 현상이 발생한 것이다.

## 3.2 시스템 외부 환경은 완전하지 않다

앞선 시나리오를 통해 알 수 있는 사실은, **제어할 수 없는 외부 요인으로 인해 예기치 못한 문제가 발생할 수도 있다**는 점이다.

이와 같이 중복 요청에 대한 문제가 발생할 수 있는 시나리오는 다음과 같다:

1. 더블 클릭
2. 불안정한 네트워크
3. 요청 처리 시간 지연

중복 요청으로 인한 리소스 변화가 어떨 때는 사용자에게 치명적인 영향을 줄 수 있다.
...
_Why?_ 에 대한 내용을 다뤘으니, 이제 이러한 의문이 들 수 있다.  
...
2. **클라이언트**  
   사실 잘 생각해보면, **현재 요청을 어떤 상황에서 보내는지**, 그 상황을 정확히 판단할 수 있는 주체는 클라이언트 뿐임을 알 수 있다. 👉🏻 ✅
...
클라이언트는 이제 멱등키를 첨부하여 서버에 요청을 보낸다.
...
1. **서버가 요청을 처리하는 도중에 중복 요청이 발생한 경우**
   ...
   서버는 재요청에 대한 로직을 처리하지 않고, `요청이 아직 처리중임`을 알리는 응답을 클라이언트에게 보낼 수 있다.

   ![idempotent-signup3](/assets/idempotent-signup3.png)

   앞선 요청에 대한 처리가 모두 완료되면, 해당 요청에 대한 진행 상태를 `COMPLETED`(완료) 상태로 표시한다. 그리고 해당 요청에 대한 응답 정보도 기록할 수 있다.

2. **서버가 요청을 처리한 이후 중복 요청이 발생한 경우**

   ![idempotent-signup4](/assets/idempotent-signup4.png)

   마찬가지로 서버는 재요청에 대한 로직을 처리하지 않고, 저장된 응답을 클라이언트에게 보낼 수 있다.

### 4.2.3 정리

위 시나리오에서 확인할 수 있는 핵심 내용은 다음과 같다:
...
시스템 내부에서 멱등성을 보장하기 위해 추가적으로 고려해야 할 사항이 늘어난다.

> 특히 멱등키를 위한 저장소가 추가됨에 따라 **In-memory 기반 저장소** 도입을 고려할 수도 있고, 동시성 문제, SPOF 문제 등과 같이 코드 복잡도 뿐만 아니라 추가로 고려해야 할 사항이 늘어난다.

1. 예시) 블로그/커뮤니티 게시글 업로드
2. 예시) 결제/재고 정보 수정
