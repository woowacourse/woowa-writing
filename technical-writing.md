...
### 'Happy Path'를 넘어, 예측 불가능성에 대비하기

프론트엔드 개발 과정은 종종 'Happy Path', 즉 모든 것이 예상대로 순조롭게 작동하는 경로를 따라 시작됩니다. 하지만 이 경로를 벗어났을 때, 즉 예측 불가능한 실패와 마주했을 때, 서비스의 완성도가 결정되곤 합니다. 이때 에러 핸들링은 단순히 버그를 수정하는 부수적인 작업이 아니라, 이런 애플리케이션의 품질을 결정짓는 중요한 척도입니다.

잘 설계된 에러 핸들링 전략은 사용자의 신뢰를 구축하고, 개발자의 생산성을 향상시키며, 서비스 전체의 안정성을 보장하는 기반이 됩니다. 사용자가 마주하는 깨진 화면이나 의미 없는 로딩 표시는 단순한 불편함을 넘어 서비스에 대한 부정적인 인식을 심어줄 수 있습니다. 반면, 개발자가 마주하는 모호한 에러 로그는 문제 해결 시간을 기하급수적으로 늘려 귀중한 리소스를 낭비하게 만듭니다.

### 이 글은 체계적인 에러 처리 아키텍처 구축을 위한 안내서입니다

이 글은 프론트엔드 에러 핸들링을 체계적으로 접근하고자 하는 개발자들을 위한 포괄적인 안내서입니다. 에러와 예외의 근본적인 개념을 명확히 하는 것부터 시작하여, 다양한 유형의 에러를 분류하고 각 상황에 맞는 방안을 소개하고자 합니다. 나아가 React와 같은 현대적인 프레임워크에서 선언적이고 우아하게 에러를 다루는 방법, 그리고 최종적으로 운영 환경에서 발생한 에러를 효과적으로 추적하고 분석하는 모니터링 시스템 구축에 이르기까지, 에러 핸들링의 전 과정을 심도 있게 다룰 것입니다.

![image2](./images/image2.png)

## 1. 기본 다지기: 에러(Error)와 예외(Exception)

<!-- TODO 텍스트 : 여기 타이틀 수정 하고 싶어 좀더 포괄적인 말로  -->
...
내장 에러만으로는 애플리케이션의 복잡한 도메인 특화적인 실패 상황을 표현하기에 부족합니다. 예를 들어, 'API 통신 실패'나 '사용자 입력값 검증 실패'와 같은 에러는 JavaScript 내장 타입으로 표현할 수 없습니다. 이때 `Error` 클래스를 상속받아 **Custom Error**를 만드는 패턴을 사용할 수 있습니다.
...
모든 에러를 동일하게 취급하려는 시도는 비효율적일 뿐만 아니라, 종종 잘못된 해결책으로 이어집니다. 견고한 에러 핸들링 시스템을 설계하기 위해서는 먼저 우리가 마주할 에러들의 종류를 파악하고 지도를 그리는 작업이 필요합니다. 프론트엔드에서 발생하는 에러는 그 근원에 따라 크게 세 가지 범주로 나눌 수 있습니다.
...
이러한 런타임 에러는 본질적으로 코드의 버그를 의미합니다. 따라서 처리 전략의 핵심은 사용자 경험 보호와 신속한 버그 수정입니다. 에러가 발생했을 때 애플리케이션 전체가 중단되는 것을 막고, 동시에 개발자가 문제를 정확히 인지하고 수정할 수 있도록 에러 모니터링 시스템에 상세한 로그를 남기는 것이 중요합니다.

### 2-2. UI 렌더링 에러

이 유형의 에러는 React, Vue, Svelte와 같은 현대적인 UI 프레임워크의 생명주기나 렌더링 과정에서 발생하는 특수한 문제들입니다. 프레임워크가 관리하는 상태(state)와 UI의 불일치가 생기거나, 프레임워크의 규칙을 위반했을 때 발생합니다.

- React 렌더링 중 에러:

  컴포넌트의 렌더 함수 내에서 `TypeError`와 같은 JavaScript 런타임 에러가 발생하면, React는 해당 컴포넌트 트리의 렌더링을 중단합니다. 이 에러를 처리하지 않으면 애플리케이션 전체가 흰 화면으로 변하며 멈출 수 있습니다.

- 상태 업데이트 관련 에러:

  무한 루프를 유발하는 상태 업데이트(예: `useEffect` 내에서 의존성 배열 없이 `setState`를 호출)나, 아직 마운트되지 않은 컴포넌트의 상태를 업데이트하려는 시도 등이 여기에 해당합니다.

- 프레임워크 규칙 위반:

  React에서 Hooks를 조건문 안에서 호출하는 것과 같이 프레임워크가 정한 규칙을 어겼을 때 발생하는 에러입니다.

이러한 렌더링 에러는 UI의 특정 부분을 '오염'시키는 특성이 있습니다. 따라서 처리 전략의 핵심은 **오염 전파 차단**입니다. 문제가 발생한 UI 영역을 격리하고, 나머지 정상적인 부분은 계속해서 사용자에게 기능을 제공할 수 있도록 해야 합니다. React의 `ErrorBoundary`는 바로 이러한 '방화벽' 역할을 수행하기 위해 설계된 메커니즘입니다.

### 2-3. 네트워크 및 API 통신 에러

프론트엔드 애플리케이션은 대부분 외부 서버와 데이터를 주고받으며 동작합니다. 이 통신 과정에서 발생하는 에러는 애플리케이션 외부의 요인에 의해 발생하는 경우가 많아 가장 예측하기 어렵고 다루기 까다로운 유형입니다.

- HTTP 에러 상태 코드:

  서버는 클라이언트의 요청에 대한 처리 결과를 HTTP 상태 코드로 응답합니다. 4xx 계열(예: 404 Not Found, 403 Forbidden)은 클라이언트 측의 요청 오류를, 5xx 계열(예: 500 Internal Server Error, 503 Service Unavailable)은 서버 측의 문제를 의미합니다.  
...
프론트엔드 애플리케이션에서 가장 빈번하고 예측 불가능한 에러의 원천은 바로 서버와의 통신 계층입니다. 사용자의 네트워크 상태, 서버의 안정성, API 응답 형식 등 수많은 변수가 존재하기 때문입니다. 따라서 이 통신 계층에 견고한 방어선을 구축하는 것은 서비스 안정성의 핵심입니다. 이번 글에서는 `axios` 라이브러리를 중심으로, 반복적인 코드를 줄이고 시스템의 회복탄력성을 높이는 고급 API 에러 핸들링 전략을 자세하게 다룹니다.
...
**ErrorBoundary**는 자신의 자식 컴포넌트 트리에서 발생하는 JavaScript 에러를 잡아내고, 에러가 발생했을 때 UI가 완전히 깨지는 대신 미리 준비된 대체 UI(Fallback UI)를 보여주는 React 컴포넌트입니다. 이는 마치 컴포넌트를 위한 `try/catch` 블록처럼 동작합니다.
...
`ErrorBoundary`는 매우 강력하지만 만능은 아닙니다. 이것이 **처리할 수 없는 에러의 종류**를 명확히 이해하는 것이 중요합니다.  
...
### `Suspense`와 `ErrorBoundary`의 환상적인 조합
...
여기서 중요한 점은, 만약 `Suspense` 모드를 사용하는 데이터 페칭 `Promise`가 성공적으로 `resolve`되지 않고 `reject`된다면, React는 이를 에러로 취급하여 **throw**합니다. 렌더링 중에 발생한 이 throw는 가장 가까운 상위 **ErrorBoundary**에 의해 잡히게 됩니다.  
...
위 예시에서 `UserProfile` 컴포넌트는 `isLoading`, `isError`, `error와` 같은 상태를 전혀 관리하지 않습니다. 오직 데이터를 성공적으로 가져왔을 때의 UI 렌더링에만 집중합니다. 로딩 상태는 상위의 `<Suspense>`가, 에러 상태는 상위의` <ErrorBoundary>`가 각각 책임지고 처리합니다.

이처럼 `Suspense`와 `ErrorBoundary`의 조합은 데이터 페칭과 관련된 상태 관리 로직을 컴포넌트로부터 완전히 분리하여, '관심사의 분리(Separation of Concerns)' 원칙을 구현합니다. 이는 컴포넌트를 훨씬 더 단순하고, 예측 가능하며, 재사용하기 쉽게 만들어 주어 개발 경험을 획기적으로 개선하는 패러다임입니다.
...
따라서 백엔드에서 전달된 에러 메시지라 할지라도, 프론트엔드는 이를 사용자가 이해할 수 있는 언어로 번역하고 정제하여 보여줄 최종적인 책임이 있습니다.
...
## 6. 운영 환경에서의 에러 모니터링과 로깅

운영 환경에서는 우리가 예측하지 못한 수많은 에러들이 사용자의 다양한 디바이스와 네트워크 환경 속에서 발생합니다. 개발자의 컴퓨터에서는 재현되지 않는 '에러'들을 찾아내고 해결하기 위해서는 체계적인 모니터링 및 로깅 시스템이 반드시 필요합니다.

## 7. 마무리: 장애는 당연합니다, 중요한 것은 회복 탄력성!

해당 글을 통해 프론트엔드 에러 핸들링이 단순히 try/catch 구문을 추가하는 행위를 넘어, 사용자 경험, 시스템 아키텍처, 그리고 운영 철학까지 아우르는 깊이 있는 분야임을 알수있었습니다. 소프트웨어 개발에서 장애는 피할 수 없는 현실입니다. 완벽하게 에러 없는 코드를 작성하겠다는 목표는 비현실적일 뿐만 아니라, 오히려 변화와 혁신을 저해할 수 있습니다. 진정한 목표는 에러가 발생하지 않는 시스템이 아니라, 에러가 발생하더라도 우아하게 실패하고(fail gracefully), 신속하게 복구하며(recover quickly), 그 과정에서 명확한 교훈을 얻어 더 나은 방향으로 진화하는 회복탄력성(Resilience) 있는 시스템을 구축하는 것입니다.
