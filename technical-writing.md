...
- scrollTop, clientHeight 등 스크롤 위치를 계산하는 복잡한 로직 없이, 요소가 뷰포트 영역에 들어왔는지 감지하여 브라우저가 정확하게 계산을 수헹합니다.

<br>

저는 실제로 우아한테크코스의 레벨 1 미션을 수행하며 이 Intersection Observer의 필요성을 느낀 적이 있는데요, 바로, 무한 스크롤 기능을 구현할 때 입니다.
...
미션을 수행할 당시에 저는 Intersection Observer는 활용하여 무한스크롤 기능을 구현했습니다. 성능적인 측면에서 추가로 고려해야할 부분이 적었고, 정확한 가시성 판단을 통해 예상치 못한 동작을 방지할 수 있었기 때문입니다.
...
이에 따라 봄봄에서도 모바일 환경의 뉴스레터 보관함에서 Intersection Observer를 활용한 무한스크롤로 지난 뉴스레터 목록을 제공하고 있습니다.

현재 봄봄은 React와 Tanstack query를 사용하고 있어, 아래와 같이 비교적 간단한 코드만으로 코드를 구현할 수 있었습니다.
