...
위와 같은 코드는 정책과 강결합돼 있습니다. 가격을 계산하는 과정에서 할인율이라는 정책이 심하게 간섭하고 있는 상황이에요. 키오스크 객체가 할인율을 신경 쓰지 않고, 외부에서 할인율을 받아오도록 한다면 의존성을 느슨하게 가져갈 수 있습니다.
```java
...
어떤 방법을 사용해야 사용자가 불편함을 느끼지 않으면서 뒤쪽 구조를 개선해 낼 수 있을까요? 한 번에 옮기는 것은 불가능해 보이니, 단계적으로 배포를 여러 번 거치면서 원하는 구조로 만들어야 합니다. 당장 추가해도 문제없는 테이블을 생성한 뒤, 서서히 마이그레이션을 시도해 봅시다. 한 번에 여러 곳으로 값이 삽입되는 것이 문제가 될 수 있지만, 저희 서비스는 **작성한 리뷰를 삭제/수정할 수 없다는 정책**이 있었습니다. 이 정책 덕에 더 쉬운 마이그레이션을 이끌어낼 수 있었어요.
