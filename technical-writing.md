...
`“스프링 CORS”`라고 검색했을 때, 스프링 환경에서 CORS를 다루는 글은 검색되지 않았습니다.
**완전 일치 방식으로는 유연한 검색이 불가능**했기 때문입니다.

2-gram 파서를 이용해 단어를 두 글자 단위로 색인했고, 매칭되는 토큰 수에 비례해 점수를 매겼습니다.
...
형태소 분석은 **복합 명사**를 분리하기도 합니다. 
예: `“로그아웃”` → `“로그”`, `“아웃”`

이 경우 의미가 훼손되므로, 사용자 사전을 통해 `“로그아웃”`을 **분리되지 않는 하나의 단어**로 등록해야 합니다.
...
이런 변형을 인식하기 위해 **동의어 사전(Synonym Dictionary)** 을 구축합니다.  
...
단순히 단어가 많이 등장하는 문서가 아니라, **가치 있는 문서가 상위**에 오르도록 해야 합니다.

이를 위해 단어 빈도 대신 문서의 가치를 점수화하려 시도했습니다.
하지만 **가치**는 매우 추상적인 개념이었고,
소수의 인원이 관련도를 수동으로 평가하는 방식은 주관적일 수밖에 없었습니다.

따라서 현재는 **검색어의 위치 기반 점수화**만 적용했습니다.
예를 들어 검색어가 제목에 포함된 경우 더 높은 점수를 부여합니다.
제목은 글의 핵심 내용을 담을 가능성이 높기 때문입니다.

## 마무리

처음에는 검색을 단순한 기능으로 생각했습니다.
하지만 실제 사용자가 기대하는 것은 **원하는 정보를 빠르고 정확하게 찾는 경험**이었습니다.

이번 개선에서는 형태소 분석, 사용자 사전, 동의어 사전 등을 통해 검색 품질의 기본 토대를 마련했습니다.

이제부터는 이를 **객관적으로 측정하고 지속적으로 개선**하는 단계로 나아가려 합니다.
...
지속적인 데이터 분석과 피드백을 통해 ‘사용자가 찾고자 하는 정보를 정확히 전달하는 검색’을 목표로 발전시켜 나갈 계획입니다.
