...
초기에는 React Context로 데이터를 공유했습니다. 하지만 기능이 늘어나자, 작은 변화에도 전체 화면이 다시 그려지는 구조 그리고 전체 refetch가 반복되는 구조가 되었습니다.
...
동선, 장소목록, 지도 순으로 3개의 영역으로 구성되어있습니다.
...
초기에는 필요한 부분만 선택적으로 감싸는 형태를 목표로 했지만, 결국 모든 컴포넌트가 두 Provider의 데이터를 함께 사용하면서 전역 Provider 구조가 되어버렸습니다.

이로 인해 Context 계층이 깊어지고, 특정 데이터만 분리해서 재활용하기도 어려워졌습니다.

### 문제 3 - 서버 상태와 클라이언트 상태 혼재

Context 내부에서는 서버에서 받아오는 데이터와 UI 렌더링을 위한 로컬 상태가 함께 관리되고 있었습니다.
...
상태의 책임이 모호해지면서, 변경 시 예기치 않은 리렌더링이나 의도치 않은 데이터 흐름이 발생하기도 했습니다.
