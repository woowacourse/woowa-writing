...
- **네트워크 요청:** 사용자가 특정 화면에 진입해야만 필요한 데이터를 수백 밀리초(ms)가 걸리는 API(Application Programming Interface) 호출로 미리 가져오는 경우.
...
1. **성능 최적화:** 불필요한 리소스 사용을 막아 애플리케이션 반응성을 높이고, 초기 로딩 시간을 줄여 사용자의 대기 시간을 단축합니다.
2. **효율적인 리소스 관리:** 당장 쓰지 않는 객체를 메모리에 올리지 않아, 메모리 사용량을 수십 메가바이트(MB)까지 절약할 수 있습니다.
...
- **생명주기(Lifecycle)에 따른 초기화 시점 관리:**
  ...
  특히 Android처럼 프레임워크가 객체의 생성과 소멸(**생명주기, Lifecycle**)을 관리하는 환경에서는, 클래스 생성 시점에 필요한 모든 리소스가 준비되지 않을 수 있습니다.
...
- **순환 참조(Circular Dependency) 문제 회피:**
  객체 A가 B를, 동시에 B가 A를 필요로 하는 **순환 참조(Circular Dependency)** 상황이 발생할 수 있습니다. 생성자(Constructor) 주입만으로는 이 문제를 풀기 어렵지만, 지연 초기화를 사용하면 일단 객체를 생성한 뒤 나중에 의존성을 주입하여 순환 참조 관계를 해소할 수 있습니다.
...
- **약간의 오버헤드(Overhead):** `Lazy` 위임 객체 생성 및 `SYNCHRONIZED` 모드의 경우 락 메커니즘으로 인한 성능 오버헤드가 발생할 수 있습니다. 하지만 일반 프로퍼티 접근에 비해 추가되는 객체 생성 및 메소드 호출 시간은 ns(나노초) 단위이므로, 대부분의 애플리케이션에서 성능 차이를 측정하기는 어렵습니다.
