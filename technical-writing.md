...
우리는 이걸 너무 당연하게 여기지만,
그 짧은 순간 동안 사실 수많은 네트워크 단계가 동시에 일어난다.

**이 글에서는 그 ‘보이지 않는 찰나’를 들여다보려 한다.**

사용자가 URL을 입력한 뒤, 브라우저가 서버와 연결되고
결국 웹페이지가 화면에 표시되기까지의 과정을 단계별로 따라가 본다.
...
![](https://velog.velcdn.com/images/wndkgus0225/post/6f7717f7-e48b-4e49-a8f4-fc9665892d41/image.png)
...
2. `www.blog.naver.com`: 도메인 - 서버의 이름 또는 위치
3. `:443`: 포트 번호 - 서버 내부의 세부 경로
...
하지만, 우리가 접속할 사이트를 보면, `https://blog.naver.com:443` http가 아닌 https를 연결해야하하는데, 먼저 이 둘의 차이점을 한 번 살펴보자!

![](https://velog.velcdn.com/images/wndkgus0225/post/0dc52109-0475-48e6-822d-cbf7b57811fe/image.png)

다음 상황은 사용자가 브라우저에 비밀번호를 입력하고, 이 비밀번호가 웹 서버로 넘어가는 과정을 모식화해보았는데,
...
방금 봤던 내용이 url을 검색하는 것부터 시작해서 브라우저 -> 서버로 이동하는 이 부분이었다.

그리고, 서버로부터 데이터를 받아오는 경우를 보면, 전과 동일하다.

![](https://velog.velcdn.com/images/wndkgus0225/post/c00d4ccd-f555-4132-a918-5bfd3f20b00a/image.png)

서버가 브라우저에게 데이터를 전달해주는 마지막 흐름을 보면
이전에 생성해놓은 TCP + TLS 통로를 거쳐 응답값을 받는다!

<br />

**이렇게 브라우저와 서버가 요청과 응답을 주고받는 사이클을 통해
하나의 네트워크 연결(통로)을 형성하게 된다.**
