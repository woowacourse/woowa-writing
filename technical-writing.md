...
- 들어가는 말에는 존댓말로 자기소개를 할것임
- 데벨업에서 만난 동시성 문제를 어쩌구했던 과정을 이해하기 쉽게 쿠폰 발급 예시로 하겠다고 밑밥
- 근데, 여기서 이제 양해를 구할거임 -> 참고: 문장의 간결함을 위해서 높임말은 생략할게요. (당근마켓 기술블로그 - MySQL Gap Lock 다시보기 참고)
...
* [제목 미정](#제목-미정)
    ...
    * [2. 문제에 어떻게 접근할 것인가?](#2-문제에-어떻게-접근할-것인가)
        * [어떻게 문제를 해결할 수 있을까?](#어떻게-문제를-해결할-수-있을까)
        * [어떤 기준으로 선택할 것인가?](#어떤-기준으로-선택할-것인가)
    * [3. 해결책을 저울질하자](#3-해결책을-저울질하자)
    ...
    * [4. 해결책을 결정해보자](#4-해결책을-결정해보자)
    * [참고](#참고)

<!-- TOC -->

## 1. 쿠폰 발급 API로 알아보는 동시성 문제

### 동시성 문제란 무엇인가

- 동시성 문제는 동시성이 문제인 것임
- 이곳에 동시성에 대한 개념을 작성할 것임
- 딥하게 설명하지는 말고

### 동시성 문제 사례 - 쿠폰 발급 API

- 이곳에 쿠폰 발급 요구사항과 간략한 코드와 설명을 할 것임
- 테스트 환경과 컴퓨터 스펙을 설명
- 단일 테스트에서는 잘되는데
- 하.. 멀티 스레드 테스트에서는 실패하는 것임ㅋㅋ

### 쿠폰 발급 API 원인 부검

- [이해하기 쉽게 그림을 넣을 거임]
- 쿠폰 발급 service에서 발생하는 동시성 문제 원인을 파악할 것임
- ~~해서 사용자가 동시 요청했을 때, 중복 발급이 된것임

## 2. 문제에 어떻게 접근할 것인가?

> 독자가 궁금해할 부분은 아닐테니, 간결하게, 선택지 많고 비용 제일 적은거 선택하는게 좋다는 걸 강조

### 어떻게 문제를 해결할 수 있을까?

- 동시 처리 능력을 희생 시킬 수 있는거라면 시도해볼만한 방식일 것임

### 어떤 기준으로 선택할 것인가?

> 적고보니 필요한 부분인지는 잘 모르겠음

- 그러면 엄청 많을텐데... 어떡하지? -> 기준이 있어야함 !
- 다양한 방식 중에 해결 방식을 결정하기 위해서는 기준이 있어야 함
- 기준은 비용이고, 성능(처리량), 사용자 경험, 다른 개발자에게 주는 인지비용, 구축 비용, 자원을 효율적으로 사용(멋진 용어 찾기)이 있다.
- 가장 적은 비용으로 효과적인 방법을 찾아야 한다.
- 이제 기준도 알아봤으니 여러가지 아이디어를 시도해보고 장단점을 알아봅시다ㄱㄱ

## 3. 해결책을 저울질하자

> 코드는 레포에 있다고 언급, 방법이 많아서 몇 가지는 버려야할듯?, 방식을 나열하는 순서도 중요할듯?
...
- [이곳에 장단점을 정리한 표 삽입]
- 데벨업에서 어떤 근거로 어떤 해결책을 선택했는지 작성
- 동시성 문제를 막기로 결정해도 다양한 선택지 중에서 비교해서 결정해야 하는데, 사실은 막아야하는지 막지 말아야할지 조차도 비교해서 결정해야함
    - github은 따닥을 안막는 부분도 있음 -> 게시글 이런거 따닥 막아야하나?
