...
```
앞서 전략 패턴에 대해 알아보았어요. 내부에 전략을 포함하는 것이 아니라 외부에서 이를 주입하도록 하는 것이 그 목표였습니다. 이로써 의존성을 느슨하게 가져가거나, 온전히 끊어낼 수도 있어요. 스프링 프레임워크에서는 빈 컨테이너를 관리하며 의존성 제어를 진행합니다. 실제로 `@Autowired`로 외부에서 객체를 주입받죠. 어플리케이션 개발자는 클래스 그 자체의 관심사에만 집중할 수 있습니다. 
...
어떤 방법을 사용해야 사용자가 불편함을 느끼지 않으면서 뒤쪽 구조를 개선해낼 수 있을까요? 한 번에 옮기는 것은 불가능해 보이니, 단계적으로 배포를 여러 번 거치면서 원하는 구조로 이끌어내야 합니다. 당장 추가해도 문제없는 테이블을 생성한 뒤, 서서히 마이그레이션을 시도해 봅시다. 한 번에 여러 곳으로 값이 삽입되는 것이 문제가 될 수 있지만, 저희 서비스는 **작성한 리뷰를 삭제/수정할 수 없다는 정책**이 있었습니다. 이 정책 덕에 더 쉬운 마이그레이션을 이끌어낼 수 있었어요.

우선 `Answer` 테이블을 추가하는 것은 큰 무리가 없어 보입니다. 실질적으로 열이 바뀌는 테이블은 `CheckboxAnswer`와 `TextAnswer` 테이블입니다. 해당 테이블에서 `questionId`, `reviewId`가 모두 `Answer` 테이블로 옮겨졌어요. 

1. `new_review`, `answer`, `new_text_answer`, `new_checkbox_answer`, 그 외 연관관계 중간 테이블 새로 생성
2. 마이그레이션 대상 이전 테이블과 현재 테이블의 `auto_increment` 값을 일정 값으로 설정 
3. 어플리케이션 배포와 함께, 앞으로의 데이터가 새로 만든 테이블과 함께 삽입되도록 설정
4. 어플리케이션 실행 중, 데이터베이스에서 설정한 `auto_increment`보다 낮은 `ID`를 가지는 행들을 새로운 테이블에 맞게 정제 후 삽입
5. 기존 레거시 코드를 삭제
...
두 번의 어플리케이션 배포가 이루어졌고, 모두 사용자가 불편함을 느끼지 않도록 최대한 신경썼어요. 당시에는 무중단 배포가 구현되어있지 않아 배포할 때마다 잠깐의 서비스 중단이 발생했겠지만, 지금은 무중단 배포도 구현되어있기 때문에 사용자가 모르는 사이에 테이블 구조를 바꿔낼 수 있겠습니다.
