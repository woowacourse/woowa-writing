...
## Hilt 학습 배경
안드로이드 미션 중에는 DI(Dependency Injection) 미션이 있었다.
이 미션에서는 기존의 Hilt 라이브러리를 사용하지 않고, 의존성 주입을 자동화하는 로직을 직접 구현해야 했다.

의존성 주입을 라이브러리 없이 자동화하는 것은 처음이었기 때문에, 처음에는 어떻게 접근해야 할지 전혀 감이 잡히지 않았다.
이에 따라, 자동화 로직을 구현하기 위해서는 Hilt의 구현 원리와 사용 방식을 먼저 이해해야 한다고 판단했다.

## Hilt를 사용한 의존성 주입

Hilt는 안드로이드용 의존성 주입 라이브러리로, 프로젝트에서 수동으로 의존성 주입을 할 때 발생하는 보일러플레이트 코드를 줄여준다. 

수동으로 의존성을 주입하는 경우, 모든 클래스와 그 클래스의 객체를 직접 생성해야 하며, 의존성을 재사용하고 관리하기 위해서는 다음과 같이 컨테이너를 사용해야한다.
...
Hilt는 애플리케이션에서 의존성을 사용할 수 있는 방법을 제공하는데, 이는 프로젝트의 모든 클래스에 대해 컨테이너를 제공하고, 이 컨테이너들의 생명주기를 자동으로 관리하는 방식이다.

컨테이너는 Hilt가 안드로이드 컴포넌트의 생명주기와 맞춰 자동으로 생성해주며, 의존성을 보관하는 상자라고 생각하면 될 듯하다.

Hilt는 널리 사용되는 DI(Dependency Injection) 라이브러리인 Dagger 위에서 구축되어 있다.
...
Hilt는 Dagger 의존성 주입(DI) 라이브러리 위에 구축된 도구로,
Android 애플리케이션에 Dagger를 표준화된 방식으로 통합할 수 있게 도와준다.
