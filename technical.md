...
또한 버전은 `LTS 24.04` 로 하였고 인스턴스 타입은 `t4g.micro` 를 하기 위해 아키텍처로 `Arm` 을 선택하였습니다.
...
> #### Key file
>
> EC2 생성 도중 `Key pair` 라는 것을 만들게 되는게 `.pem` 파일 하나가 다운로드 되는데 `SSH` 접속에 꼭 필요한 파일이니 홈 폴더 아래의 `.ssh` 폴더에 넣어두시면 관리가 편할거에요!
...
명령어를 입력한 후 첫줄에 `Welcome to Ubuntu 24.04 LTS (GNU/Linux 6.8.0-1013-aws aarch64)` 같은 메세지가 보이고 마지막 줄에 초록색 으로 `ubuntu@ip-10-0-0-67:~$` 가 보인다면 접속에 성공한 것입니다.
...
이전 단계에서 `[ip.address]` 를 직접 모두 입력하여 접속하는 것이 불편할 뿐더러 `EC2` 가 종료되었다 켜지면 이 `[ip.address]` 가 변경되어 외우기 곤란합니다.

또한 웹 서버 접속 시 `https://3.34.42.134` 과 같이 접속하지 않고 `https://dev.pengcook.net` 으로 접속합니다.
