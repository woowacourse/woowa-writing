...
useSyncExternalStore로 다시 생각한 전역 상태 관리

## 단순한 기능이 드러낸 구조적 복잡성

토스트 팝업은 가장 단순한 형태의 피드백 UI입니다.
사용자 행동 후 잠깐 나타났다 사라지는 메시지에 불과하지만, 막상 구현에 들어가면 금세 단순하지 않다는 걸 깨닳았습니다.

**구현하면서 고민했던 사항은 다음과 같습니다.**
...
useSyncExternalStore로 접기하기 이전에 2가지를 고민했습니다.

**첫번째,** 일반적으로 많이 사용하는 Context API 방식이었습니다.
...
**두번째,** 전역 상태 라이브러리(`Redux, Zustand`)를 도입하게 된다면 위와 같은 부분을 해결하기 위할 수도 있지만, 작은 기능을 위해 전역 스토어를 세팅하는 건 과하다고 생각을 했습니다.
