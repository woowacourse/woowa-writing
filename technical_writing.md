...
이러한 상황을 팬텀 리드(Phantom Read)라고 합니다. 참고로 MySQL은 `REPEATABLE READ` 이상의 격리 수준에서만 팬텀 리드를 방지하기 위해 갭 락을 겁니다. 여기서는 팬텀 리드에 대해 간략하게 설명만 하고 넘어가지만, 잘 모르신다면 추가적으로 학습해 보시길 바랍니다. 
