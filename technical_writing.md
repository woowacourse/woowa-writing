...
MySQL에서 자동 증가 키를 사용하면 거의 모든 상황에 `IDENTITY` 전략을 사용하게 됩니다. Hibernate를 사용하는 환경에서 특정 API가 대량의 데이터를 다뤄야 하는 상황이 생기면 어떻게 해야할까요? 새로운 기술을 도입하거나, 키 생성 전략을 변경할 수도 있지만 순수 JDBC를 사용해서 문제를 해결할 수 있습니다.

JPA는 JDBC 위에서 동작하는 추상화된 API 입니다. 당연하게도 JPA가 제공하는 기능들은 모두 내부적으로 JDBC 기반으로 동작하고 있습니다. 따라서 JPA를 사용하고 있으면 다른 기술의 도입이나 변경 없이 JDBC를 사용할 수 있고, 당연히 JDBC를 활용한 Batch처리를 활용할 수 있습니다.
...
현재 모모 서비스에서 새로운 일정 등록 시 사용되고 있는 코드입니다. JDBC의 `batchUpdate()` 메서드를 사용해서 단건으로 나가던 쿼리를 `INSERT INTO ... VALUES(..), (..), (..);` 개선할 수 있습니다.
...
위는 JPA, JDBC를 사용했을 때 데이터 개수에 따른 성능 변화를 나타낸 것입니다. 처리해야 할 데이터 수가 많아질 수록 JPA와 비교해서 JDBC의 `batchUpdate()`를 사용했을 때 성능 차이가 100배에 달하는 것을 확인할 수 있습니다.
...
`MySQL Packet Size Limit`는 MySQL 명령어로 쉽게 확인할 수 있고, 기본 값은 `67108864B`, `64MB`입니다.
따라서 하나의 레코드 사이즈를 계산할 수 있으면 한 번에 몇 개 데이터까지 전송할 수 있는지 대략적으로 파악할 수 있습니다.
  
1000만개가 들어있는 Schedule 테이블의 data length는 711,983,104B 입니다. 이를 계산해봤을 때 하나의 레코드당 대략 71B 임을 알 수 있습니다. 67108864B/71B 를 하면 945,195. 약 94만개 까지는 한 번에 전송할 수 있다는 대략적인 수치를 얻을 수 있습니다. 이 계산 방법은 예시이며 실제로 큰 BatchSize를 사용해야 할 때는 부하 테스트를 통한 지속적인 모니터링이 필수입니다.
  
현재 모모 서비스는 예약을 생성하는 과정에서 날짜들을 생성하는 경우 BatchSize를 30, 일정을 생성하는 경우 BatchSize를 500으로 설정하였습니다. Batch Processing의 철학을 고려해보면 현재보다 사이즈를 크게 설정해도 무리가 없을 것으로 예상하지만,
아직 실제 환경에서의 부하 데이터가 없고, OLTP 환경임을 고려했을 때 조금 더 보수적으로 설정하는 것이 안전하다고 판단했습니다.

## 마무리

JPA와 MySQL을 사용하는 환경에서 다량의 데이터를 삽입해야 할 시 발생하는 문제점과 해결 방안을 소개하였습니다. "은총알은 없다" 라는 말이 있습니다. 운영하는 서비스마다 서버 환경이 다르니 적절한 부하 테스트와 지속적인 모니터링을 통해 적절한 BatchSize를 결정하는 것이 중요할 것 같습니다.
