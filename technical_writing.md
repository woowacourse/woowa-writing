...
```
`main()` 메서드를 실행했을 때 결과를 예측해보자. 실행 결과는 아래 ***서론의 예제 분석***에서 다룰 것이다.

여기선 Java를 예시로 들었지만 흥미로운 사실은 개발 언어마다 실행 결과가 다르다는 것이다.
...
먼저 `main()` 메서드에서 호출한 변수 `parameter`가 콜 스택에 쌓인다. 이후 `process()` 메서드가 호출되고 Return Address가 저장된다. Return Address란 메서드가 실행을 마치고 다시 돌아가야 할 위치를 저장하는 메모리 주소이다. 

다음으로 파라미터인 변수 `value`가 콜 스택에 쌓인다.
...
```
`main()` 메서드를 실행해보면 Pass by Value와 다른 결과가 도출된다.
...
`process()` 메서드에서 파라미터로 전달 받은 변수 `value`는 변수 `parameter`가 저장된 값의 참조이다. 
...
자바 명세인 JLS에 Java는 Pass by Value라 명시되어 있다.

하지만 자바는 기본적으로 원시 타입과 참조 타입이 존재한다.
두 타입은 어떻게 Pass by Value를 사용할까?
이를 위해 Java의 메모리 할당 방식에 대한 이해가 필요하다.
...
참조 타입인 객체는 파라미터로 전달될 때 내부 연산은 가능하지만 그 객체 자체가 새로운 객체로 재할당하는 것은 반영이 되지 않는다.
