...
Spring Boot에서는 크게 세 가지 방식으로 채팅을 구현할 수 있다. 비실시간 방식인 폴링, 그리고 실시간 방식인 롱 폴링과 웹소켓이다. 이 글에서는 각각의 방식의 차이를 간단하게 알아보고, 웹소켓을 사용하여 실시간 채팅 프로그램을 구현하는 방법을 설명한다. 이 글은 중급 이상의 Java 문법과 Spring Boot의 기본 작동 원리를 이해하고 있는 사람들을 대상으로 한다.
...
서버와 클라이언트의 웹소켓 과정은 다음과 같다. 먼저, 클라이언트는 웹소켓이 사용 가능한지 확인하기 위해 Upgrade 헤더를 포함하여 서버로 요청을 보낸다. 서버는 웹소켓 사용이 가능한 경우 101 상태 코드를 응답하여 클라이언트와 웹소켓 연결을 맺는데, 이 과정을 오프닝 핸드셰이크라고 한다.

핸드셰이크 과정이 성공적으로 끝나면 서버와 클라이언트는 ws 혹은 wss 프로토콜을 사용하여 양방향 통신이 가능하다. http 및 https와 동일한 포트 번호를 사용하기 때문에, 포트
...
STOMP는 기본적으로 publish - subscribe 구조를 따른다. 따라서 STOMP를 이해하려면 publish와 subscribe가 무엇인지 이해할 필요가 있다.
...
두 개의 서로 다른 탭에서 [http://localhost:8080/](http://localhost:8080/%EC%97%90)에 접속한 뒤, Connect를 누르고 메시지를 전송해 보자. 채팅 메시지가 서로 실시간으로 반영되는 것을 확인할 수 있다.
...
public void configureClientInboundChannel(ChannelRegistration registration) {
    registration.interceptors(webSocketInterceptor); // 주입 필요
}
```

`ChannelInterceptor`를 구현하면 웹소켓으로 전송되는 메시지들을 가로채 특정 로직을 수행하도록 지정할 수 있다. STOMP에서 구독에 사용되는 `SUBSCRIBE` 커맨드로 들어오는 메시지를 가로채 특정 조건에 따라 예외를 발생시키도록 작성하고, 인터셉터를 설정 파일에 등록하면 구독 과정에서의 예외를 처리할 수 있다.
