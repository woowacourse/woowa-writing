...
이러한 상황을 팬텀 리드(Phantom Read)라고 합니다. 
...
앞에서 본 것처럼 갭 락은 특정 레코드 **앞의 간격**(왼쪽)에 대해서만 잠금을 걸 수 있기 때문에, 인덱스에서 **가장 큰 레코드 이후의 간격에는 갭 락을 걸 수 없습니다**.  

따라서 인덱스의 **가장 큰 레코드 이후의 값 삽입을 제어하기 위해** supremum pseudo-record X 락을 사용합니다.
...
좀 더 정확히 설명하자면 모든 레코드에 레코드 락(해당 레코드에만 걸리는 락)이 걸리고, 또한 모든 레코드 사이의 간격에 갭 락이 걸리게됩니다.
**이렇게 레코드 자체에 대한 락인 레코드 락과, 해당 레코드 왼쪽에 삽입을 방지하는 갭 락을 합쳐서 넥스트 키 락이라고 합니다.** 

넥스트 키 락은 X 락으로 설정될 때 서로 공유할 수 없기 때문에 위의 상황에서 데드락이 발생하지 않는 것입니다. 넥스트 키 락에 대한 설명이 추가되면 내용이 길어질 수 있으므로 여기서는 간단한 개념만 짚고 넘어가도록 하겠습니다.


하지만 테이블이 비어 있는 경우에는 데드락이 발생합니다. PK 인덱스를 스캔해도 결과가 나오지 않게 되며, 이때 supremum pseudo-record X 락이 걸리면서 여러 트랜잭션이 동시에 획득할 수 있게 됩니다:
...
인덱스 없이 X 락을 걸면 동시성 문제는 해결할 수 있지만, 이에 따라 해당 컬럼에 인덱스를 더 이상 걸 수 없다는 제약이 생기게 됩니다. unique를 걸게 되면 인덱스가 생성되기 때문에, 해당 컬럼들에 대해 unique 제약 조건도 걸 수 없게 됩니다. 이는 큰 단점이라고 생각했습니다. 

또한, `SELECT ... FOR UPDATE`는 동시성이 매우 떨어지는 방식이기 때문에 성능 저하 문제도 발생할 수 있습니다.
...
또한, INSERT IGNORE를 활용한 해결책이 중복된 유일 키나 기본 키 삽입 시 성능 저하 없이 에러를 방지하며 데이터의 정합성을 유지할 수 있는 효과적인 방법임을 확인했습니다. 이러한 경험이 여러분이 직면할 수 있는 유사한 문제를 해결하는 데 큰 도움이 되길 바랍니다. 언제든지 질문이나 논의가 필요하시면 주저하지 말고 말씀해 주세요!
