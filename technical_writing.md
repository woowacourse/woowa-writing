...
캐시는 단순히 응답 속도를 빠르게 만드는 기술이 아닙니다.
그 본질은 데이터베이스가 쓰기와 일관성 보장이라는 핵심 역할에 집중할 수 있도록, 읽기 부하를 대신 흡수하는 완충 장치에 가깝습니다.

빠른 응답은 결과일 뿐, 진짜 목적은 시스템 전체의 안정성과 자원의 효율적 분배에 있습니다.
캐시가 빠르다는 점보다 더 중요한 이유는, 요청 수 자체를 줄여 DB의 부담을 줄이는 것에 있습니다.

예를 들어 1초에 5,000건의 상품 상세 요청이 들어올 때, Cache Hit 비율이 90%라면 DB는 5,000건이 아닌 500건만 처리하면 됩니다.
즉, DB 커넥션, I/O, 락 경합 등의 리소스 사용량이 10분의 1로 줄어드는 셈이죠.
이처럼 **부하 분산(Load Distribution)**이 캐시의 진짜 가치이며,
이는 단순한 속도 향상보다 훨씬 근본적인 성능 개선 효과를 가져옵니다.
