...
로그인, 소셜 로그인 및 OAuth 에 대한 개념과 설명이 주를 이루고, 결론으로 CoReA 에선 어떤 방식으로 구현되었는지를 설명한다.

CoReA 팀원들이 어떤 고민을 통해서 지금과 같은 형태로 구현하게 되었는지를 알리는 것이 목표이며,

추가적으로 로그인 기능, 특히 Github OAuth 를 이용하려는 백엔드 개발자에게 프로세스에 대한 이해와 결정을 돕는 것을 목적으로 한다.
...
개인정보를 어떻게 받고, 검증할 것이며 어떤 방식으로 저장할 지 등 다양한 문제를 고민해야 한다.
...
신빙성 있는 Authorization Server, Resource Server 를 운영하는 서비스가 위 과정을 구현해 두었다면, 우리는 이 과정을 통해서 인증 작업을 위임할 수 있는 것이다.
