...
실시간 미니게임 서비스를 만들면서 웹소켓을 도입했다. REST API라면 Swagger로 명세를 자동 생성할 수 있다. 코드에 어노테이션 몇 개만 달면 알아서 API 문서가 만들어진다.

그런데 웹소켓에는 Swagger 같은 게 없었다.

정확히 말하면 웹소켓 명세 자동화의 표준이 없었다. REST API가 수십 년간 쌓아온 생태계와 달리, 웹소켓은 각자도생이었다. AsyncAPI라는 도구가 있긴 했는데, 치명적인 한계가 있었다.

YAML 파일을 수동으로 작성해야 한다는 것.

결국 Notion에 명세를 직접 작성하기 시작했다. 처음에는 괜찮았다. 이벤트 타입 5개, 개발자 3명. 이 정도는 수동으로 관리할 만했다.
...
기능 개발에 몰두하다 보니 명세 업데이트는 자꾸 뒷전이 됐다. "나중에 한 번에 업데이트하지 뭐"라는 생각이 쌓이면서, 어느새 명세서는 코드와 완전히 다른 평행세계가 되어버렸다.

클라이언트 팀의 불만이 커졌다. 명세서 업데이트 요청이 하루에도 몇 번씩 날아왔다. 프론트엔드 개발자들은 웹소켓 연결을 시도할 때마다 "이 명세 믿어도 되나요?"라고 재차 확인했다.

신뢰는 바닥을 쳤다. 더 이상 수작업으로는 안 되겠다는 생각이 들었다.

코드가 곧 명세가 되어야 했다. Swagger가 REST API에 해주는 것처럼. 코드베이스에서 자동으로 명세를 생성하는 시스템. 코드가 바뀌면 명세도 바뀌는. 100% 동기화.

하지만 그런 도구는 없었다. 직접 만들어야 했다.

이 글은 웹소켓 명세 자동화를 맨땅에서 구축한 기록이다. 삽질했던 것들, 실패했던 접근들, 그리고 마침내 찾은 해법을 담았다.
...
AsyncAPI를 발견했다. 웹소켓의 Swagger를 지향하는 오픈소스 프로젝트였다. YAML 파일로 명세를 작성하면, 멋진 GUI 문서를 자동으로 생성해준다.
```yml
asyncapi: 3.0.0
channels:
  /room/{joinCode}:
...
이 YAML을 AsyncAPI Generator에 넣으면 Swagger처럼 문서가 나온다.
...
결국 원점이었다. 코드를 수정하면 YAML도 수동으로 수정해야 하고, 그러다 보면 또 불일치가 생긴다.
...
2024년이잖아. AI가 코딩도 하는 시대에 YAML 하나 못 만들까 싶었다.

생각해보니 간단했다. 코드를 AI에게 읽혀서 YAML을 자동 생성하면 되는 거 아닌가.
...
이건 프로덕션에 쓸 수 없다. 매번 결과가 다르면 자동화의 의미가 없다.

AI로는 신뢰할 수 있는 명세 자동화가 불가능하다는 결론에 도달했다.

#### 기존 라이브러리는 없을까

AI가 안 되면 기존 라이브러리는 어떨까.
...
핵심은 Reflections 라이브러리다. 지정한 패키지 내 모든 어노테이션을 스캔할 수 있어서, 웹소켓 엔드포인트 정보를 자동으로 수집할 수 있다.

#### 3단계: YAML로 변환하기

수집한 정보를 AsyncAPI 3.0.0 형식의 YAML로 변환한다. 예를 들어 이런 코드가 있다면:
...
`List<PlayerResponse>` 를 `List_PlayerResponse` 로 변환해서 JSON 호환 키를 만든다.
...
- 경로 파라미터 → `channel` 정의에 포함
- 메시지 파라미터 → `payload` 스키마로 정의
...
간단하다. `@DestinationVariable` 어노테이션이 있으면 경로 파라미터, 없으면 메시지 파라미터다.
...
리플렉션으로 필드를 읽어서 직접 파싱할 수도 있는데, 너무 복잡하다. 제네릭, Enum, 중첩 객체 등 고려할 게 많다.

대신 라이브러리를 활용했다. victools JSON Schema Generator:
...
클래스를 넘기면 JSON Schema를 자동으로 만들어준다. Enum도 알아서 처리한다. 직접 파싱하는 것보다 훨씬 안정적이다.
...
AsyncAPI Generator CLI라는 공식 도구가 있긴 한데, 사용하지 않기로 했다.

왜냐하면:

1. **빌드 복잡도 증가**: CLI 도구는 Node.js와 CLI 설치가 필요하다. Java 프로젝트인데 Node.js를 추가로 설치하고 빌드 스크립트에 CLI 명령을 넣어야 한다. 빌드 과정이 복잡해진다. 실제로 총 빌드 시간이 2분에서 3분30초정도로 빌드시간이 1분 30초정도 느려졌다.
2. **배포 환경**: 프로덕션 서버에 Node.js를 설치하고 싶지 않았다. Spring Boot만 있으면 되게 만들고 싶었다.
...
Thymeleaf 템플릿은 어떻게 만들었을까. AsyncAPI 스펙을 보면서 직접 HTML을 짜기에는 시간이 너무 오래 걸릴 것 같았다.

그래서 AI를 활용했다. Claude에게 AsyncAPI YAML 구조를 주고 "이걸 보기 좋게 렌더링하는 Thymeleaf 템플릿을 만들어줘"라고 요청했다. 바이브코딩으로 기본 템플릿을 빠르게 생성하고, 세부적인 스타일링만 직접 수정했다.

AI는 명세 자동화에는 실패했지만, 템플릿 생성 같은 반복적인 작업에는 유용했다.

전체 흐름:

1. Spring Boot 애플리케이션 재시작
2. `AsyncApiGenerator`가 자동 실행
3. 최신 YAML 파일 생성
4. `/docs` 접속 → Thymeleaf가 YAML 읽어서 HTML 렌더링
...
관심 있다면 https://github.com/20HyeonsuLee/websocket-docs-generator 를 참고하자.
