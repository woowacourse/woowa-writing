...
Amazon S3에 파일을 업로드하는 방법 중 하나는 Java의 Stream을 사용하는 것입니다. 이 방법은 특히 서버의 디스크 리소스를 최소화하면서 대용량 파일을 효율적으로 처리할 수 있도록 도와줍니다. Stream을 이용한 업로드는 파일을 물리적으로 저장하지 않고 직접 S3에 전송하기 때문에, I/O 오버헤드를 줄이고 빠른 데이터 전송을 가능하게 합니다.

Stream 업로드는 파일을 메모리에서 직접 읽어서 S3에 바로 쓰는 방식입니다. 이 과정은 InputStream 객체를 사용하여 파일 콘텐츠를 읽고, Amazon S3 클라이언트 라이브러리의 putObject 메소드를 통해 S3 버킷에 데이터를 전송합니다. 데이터는 네트워크를 통해 스트림 형태로 전송되며, 서버의 디스크에는 어떠한 임시 파일도 생성되지 않습니다.

아래 예시 코드와 같이 HttpServletRequest의 InputStream을 s3Client까지 바로 전달하는 방식입니다.
...
웹 애플리케이션에서 사용자가 이미지 파일을 업로드하는 기능은 매우 일반적입니다. Spring Framework에서는 MultipartFile 인터페이스를 사용하여 이 과정을 손쉽게 구현할 수 있습니다. MultipartFile은 Spring의 웹 모듈인 Spring MVC에 포함되어 있으며, HTTP 요청을 통해 전송된 파일 데이터를 쉽게 처리할 수 있게 해줍니다.

아래 예시 코드와 같이 Controller는 MultipartFIle 객체를 메소드 인자로 받습니다.
...
파일의 크기, 동시 요청 수, S3 의존성, 마감기한을 고려해서 3가지 방식중에 선택해야합니다.

AWS Multipart 방식은 대용량, 동시 요청에도 우수한 성능을 보이는 방식이지만 CORS, LifeCycle 등 고려해야할 사항이 있고 요청 Flow에 대한 이해가 필요합니다.

따라서 마감기한이 충분하고 파일 스토리지를 S3에서 변경할 일이 없을 것 같다면 AWS Multipart 방식을 추천합니다.

반면에 마감기한이 충분하지 않은 상황이라면 Stream or MultipartFile 방식을 고려하는 것이 좋습니다.

이때 파일의 크기가 크지 않지만 동시 요청 수도 많다면 MultipartFile 방식을 선택하는 것이 좋을 것 같습니다.

Stream 방식은 동시요청이 많아졌을 때 네트워크 환경에 민감하기 때문입니다.

반대로 파일의 크기가 큰데 동시 요청 수가 적다면 Stream 방식을 선택하는 것이 좋습니다.

Multipart 방식은 임시 디스크를 사용하여 용량 부족 및 힙메모리 문제가 발생할 수 있기 때문입니다.
